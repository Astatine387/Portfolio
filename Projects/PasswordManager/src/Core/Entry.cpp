...
size_t Entry::deser(const uint8_t* src, size_t srcLen) {
  ...
  if (cur + dataLen > srcLen || dataLen > kMaxSiteLen)
    return 0;
  ...
  if (cur + dataLen > srcLen || dataLen > kMaxAccLen)
    return 0;
  ...
  if (cur + dataLen > srcLen || dataLen > kMaxPWLen)
    return 0;

  if (pw.setData(reinterpret_cast<const char*>(src + cur), dataLen))
    return 0;
  cur += dataLen;
