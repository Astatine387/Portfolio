...
TEST(EntryTest, DeserializeOversizedPassword) {
  ...
  EXPECT_EQ(BuildAndDeser(entry, 1, 1, kMaxEntryPwLen + 1), 0);
}
...
TEST(EntryTest, DeserializeMaxFieldLengths) {
  ...
  EXPECT_NE(BuildAndDeser(entry, kMaxSiteLen, kMaxAccLen, kMaxEntryPwLen), 0);
  EXPECT_EQ(entry.site.size(), kMaxSiteLen);
  EXPECT_EQ(entry.acc.size(), kMaxAccLen);
  EXPECT_EQ(entry.pw_len, kMaxEntryPwLen);
}
