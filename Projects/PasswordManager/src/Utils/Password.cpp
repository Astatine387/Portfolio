...
bool Password::Equal(const Password& other) const {
  ...
  size_t min_size_ = (size_ < other.size_) ? size_ : other.size_;

  for (size_t i = 0; i < kMaxPWLen; i++) {
    uint8_t a = (i >= min_size_) ? 0 : static_cast<uint8_t>(data_[i]);
    uint8_t b = (i >= min_size_) ? 0 : static_cast<uint8_t>(other.data_[i]);
